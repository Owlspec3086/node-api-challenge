...
router.get('/:id', (req, res) => {
  ...
  Db.get(id)
    ...
    .catch((err) =>
      res.status(500).json({ error: 'could not find user at that id' })
    );
});
...
router.put('/:id', (req, res) => {
  ...
  Db.get(id)
    .then((item) => {
      if (item) {
        Db.update(id, req.body)
          .then((item) => {
            Db.get(id).then((projects) => res.status(200).json(projects));
          })
    ...
    .catch(() =>
      res.status(500).json({ error: 'can not find user with that id' })
    );
});
...
router.delete('/:id', (req, res) => {
  ...
  Db.remove(id)
    .then((item) => {
      Db.get().then((projects) => res.status(200).json(projects));
    })
    .catch((err) => res.status(500).json({ error: 'can not delete user' }));
});
